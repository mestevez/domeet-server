        ...
        <v-list>
          <template v-for="(subject, index) in meetData.subjects">
            ...
            <v-list-tile :key="subject.subject_id">
              <v-list-tile-content>
                <v-layout>
                  <v-flex>{{ meetData.subjects[index].subject_title }}</v-flex>
                </v-layout>
              </v-list-tile-content>
              <v-list-tile-action>
                <v-btn v-if="meetData.subjects[index].subject_time_start == null" flat icon color="primary" @click="startSubject(meetData.subjects[index])">
                  <v-icon>play_circle_filled</v-icon>
                </v-btn>
                <v-btn v-else-if="meetData.subjects[index].subject_time_end == null" flat icon color="warning">
